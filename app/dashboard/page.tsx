...
export default async function DashboardPage() {
  ...
  const [{ data: profile }, { data: sessions }, { data: flashcards }, { data: answers }] =
    await Promise.all([
  ...
  const dueCards   = (flashcards ?? []).filter(f => new Date(f.next_review) <= new Date()).length
  ...
  const mateiraColors: Record<string, string> = {
  ...
  return (
    <div style={{ padding: '28px 32px', flex: 1, overflowY: 'auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0,1fr))', gap: '12px', marginBottom: '24px' }}>
        {[
          { label: 'Horas estudadas',    value: formatDuration(totalMin),           delta: 'Total acumulado',             color: 'var(--accent)' },
          { label: 'Flashcards para hoje', value: String(dueCards),                 delta: dueCards > 0 ? 'Pendentes' : 'Em dia!', color: dueCards > 0 ? 'var(--amber)' : 'var(--green)' },
          { label: 'Taxa de acerto',     value: `${acerto}%`,                       delta: `${totalAns} questões feitas`, color: acerto >= 70 ? 'var(--green)' : 'var(--amber)' },
          { label: 'Sessões salvas',     value: String(sessions?.length ?? 0),      delta: 'Na biblioteca',               color: 'var(--accent2)' },
        ].map(card => (
          <div key={card.label} style={{
            background: 'var(--surface)', border: '1px solid var(--border)',
            borderRadius: '12px', padding: '16px',
          }}>
            <div style={{ fontSize: '11px', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px' }}>
              {card.label}
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            ...
            recentSessions.map((s, i) => (
              <div key={s.id} style={{
                ...
                <div style={{
                  ...
                  background: mateiraColors[s.materia ?? ''] ?? 'var(--accent)',
                }} />
                <div style={{ flex: 1, fontSize: '13px', color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {s.title}
                </div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px' }}>
          ...
          {[
            { href: '/dashboard/busca',      label: 'Nova sessão de estudo',   sub: 'Buscar tema ou enviar PDF',        color: 'var(--accent)' },
            { href: '/dashboard/flashcards', label: `Revisar ${dueCards} flashcards`, sub: 'Repetição espaçada', color: 'var(--amber)' },
            { href: '/dashboard/resumos',    label: 'Biblioteca de resumos',   sub: 'Ver sessões salvas',               color: 'var(--accent2)' },
            { href: '/dashboard/ferramentas', label: 'Iniciar Pomodoro',       sub: '25min foco + 5min pausa',          color: 'var(--green)' },
          ].map(item => (
            <a key={item.href} href={item.href} style={{
              display: 'flex', alignItems: 'center', gap: '12px',
              padding: '10px 12px', borderRadius: '8px', marginBottom: '6px',
              background: 'var(--surface2)', border: '1px solid var(--border)',
              textDecoration: 'none', transition: 'border .12s',
            }}
            ...
            >
              ...
              <div>
                <div style={{ fontSize: '13px', color: 'var(--text)', fontWeight: 500 }}>{item.label}</div>
                <div style={{ fontSize: '11px', color: 'var(--muted)', marginTop: '1px' }}>{item.sub}</div>
              </div>
            </a>
          ))}
        </div>
